...
exports.getTransactionsByBank = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Failed to fetch transactions by bank",
      error: error.message,
    });
  }
};
...
exports.getTransactionsByDateRange = async (req, res) => {
  ...
  try {
    ...
    if (transactions.length === 0) {
      return res
        .status(404)
        .json({ error: "No transactions found for this date range" });
    }
    res.status(200).json(transactions);
  } catch (error) {
...
exports.getTransactionsByType = async (req, res) => {
  ...
  try {
    ...
    if (transactions.length === 0) {
      return res
        .status(404)
        .json({ error: "No transactions found for this type" });
    }
    res.status(200).json(transactions);
  } catch (error) {
...
exports.getTransactionsByStatus = async (req, res) => {
  ...
  try {
    ...
    if (transactions.length === 0) {
      return res
        .status(404)
        .json({ error: "No transactions found for this status" });
    }
    res.status(200).json(transactions);
  } catch (error) {
...
exports.createJointTransaction = async (req, res) => {
  try {
    ...
    const jointAccount = await JointAccount.findByPk(
      fromAccountId || toAccountId,
    );
    if (!jointAccount) {
      return res
        .status(404)
        .json({ error: "invaild JointAccount/JointAccount exists" });
    }
    ...
    if (!isUserPartOfJointAccount) {
      return res
        .status(403)
        .json({ error: "User is not part of the joint account" });
    }
    ...
    if (type === "withdrawal" || type === "transfer") {
      if (jointAccountBalance < parseFloat(amount)) {
        return res
          .status(400)
          .json({ error: "Insufficient balance for withdrawal or transfer" });
      }
    }
