...
const StatCard = ({ label, value, icon: Icon, color }) => (
  <div className="bg-white rounded-2xl border border-slate-100 p-6 flex items-center gap-5 shadow-sm hover:shadow-md transition-shadow duration-200">
    ...
    <div>
      ...
      <p className="text-3xl font-bold text-slate-800" style={{ fontFamily: "'Sora', sans-serif", letterSpacing: "-0.03em" }}>
        {value}
      </p>
    </div>
  </div>
...
const SuperAdminUsers = () => {
  ...
  const { data: users, loading: usersLoading, error: usersError } = useFetch("/user/get", auth?.accessToken);
  const { data: banks, loading: banksLoading } = useFetch("/bank/get", auth?.accessToken);
  ...
  return (
    <div
      ...
      style={{ background: "linear-gradient(135deg, #f0f4ff 0%, #fafbff 60%, #f0f7ff 100%)" }}
    >
      {/* Page Title */}
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-800" style={{ fontFamily: "'Sora', sans-serif", letterSpacing: "-0.02em" }}>
            Users
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <StatCard label="Total Users" value={totalUsers} icon={HiOutlineUsers} color="bg-gradient-to-br from-blue-500 to-blue-700" />
        <StatCard label="Active Users" value={activeUsers} icon={HiOutlineUsers} color="bg-gradient-to-br from-emerald-500 to-emerald-700" />
      </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                {["Name", "Email", "Phone", "Bank", "Role", "Status", "Actions"].map((h) => (
                  <th key={h} className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-widest whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-slate-50">
              {users?.map((user) => (
                <tr key={user.id} className="hover:bg-slate-50/70 transition-colors duration-100">
                  ...
                  <td className="px-5 py-3.5 text-sm text-slate-500">{user.phone || "N/A"}</td>
                  <td className="px-5 py-3.5 text-sm text-slate-500 whitespace-nowrap">{getBankName(user.bank)}</td>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        onClick={() => navigate(`/superadmin/users/update/${user.id}`)}
